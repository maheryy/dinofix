...
final class MakeCrud extends AbstractMaker
{
    ...
    {
        $command
            ->addArgument('context', InputArgument::REQUIRED, sprintf('Choose a context (e.g. <fg=yellow>%s</>)', implode(', ', MakeController::AVAILABLE_CONTEXT)))
            ->addArgument('entity-class', InputArgument::OPTIONAL, sprintf('The class name of the entity to create CRUD (e.g. <fg=yellow>%s</>)', Str::asClassName(Str::getRandomTerm())))
            ->setHelp(file_get_contents(__DIR__ . '/../Resources/help/MakeCrud.txt'));

        $inputConfig->setArgumentAsNonInteractive('context');
    ...
    public function interact(InputInterface $input, ConsoleStyle $io, Command $command)
    {
        ...
        $contextQuestion->setAutocompleterValues(MakeController::AVAILABLE_CONTEXT);

        do {
            isset($contextValue) && $io->error(sprintf('Invalid context "%s".', $contextValue));
            $contextValue = $io->askQuestion($contextQuestion);
        } while (!in_array($contextValue, MakeController::AVAILABLE_CONTEXT));
    ...
    public function generate(InputInterface $input, ConsoleStyle $io, Generator $generator)
    {
        ...
        $routeShortName = Str::asRouteName($controllerClassDetails->getRelativeNameWithoutSuffix());
        $routeName = strtolower($input->getArgument('context')) . '_' . $routeShortName;
        $templatesPath = strtolower($input->getArgument('context')) . '/' . Str::asFilePath($controllerClassDetails->getRelativeNameWithoutSuffix());
